...
class QuranDatabaseService
{
    ...
    private function formatSolrVerses(
        ...
        foreach ($resultSet as $doc) {
            ...
            $verses[] = [
                ...
                'textTajweed' => $this->stripVerseNumberFromTajweed($doc['text_uthmani_tajweed_t'] ?? null),
    ...
    /**
     * Strip embedded verse numbers (Arabic-Indic / Eastern Arabic-Indic / ASCII digits)
     * from the end of the tajweed text so they don't duplicate the UI's own verse badges.
     */
    private function stripVerseNumberFromTajweed(?string $text): ?string
    {
        if ($text === null) {
            return null;
        }

        return preg_replace('/\s*[\x{0660}-\x{0669}\x{06F0}-\x{06F9}0-9]+$/u', '', $text);
    }
